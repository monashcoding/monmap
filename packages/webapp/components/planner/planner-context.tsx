...
export function PlannerProvider({
  ...
  const [course, setCourse] = useState<PlannerCourseWithAoS | null>(defaultCourse);

  const [unitsMap, setUnitsMap] = useState<Map<string, PlannerUnit>>(
    () => new Map(Object.entries(prewarmed.units)),
  );
  const [offeringsMap, setOfferingsMap] = useState<Map<string, PlannerOffering[]>>(
    () => new Map(Object.entries(prewarmed.offerings)),
  );
  const [requisitesMap, setRequisitesMap] = useState<Map<string, RequisiteBlock[]>>(
  ...
  const switchCourse = useCallback(async (code: string) => {
    startTransition(async () => {
      try {
        ...
        if (c) {
          const codes = [
            ...new Set(c.areasOfStudy.flatMap((a) => a.units.map((u) => u.code))),
          ];
